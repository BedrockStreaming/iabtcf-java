package tech.bedrockstreaming.iabtcf.decoder;
...
import tech.bedrockstreaming.iabtcf.exceptions.InvalidRangeFieldException;
import tech.bedrockstreaming.iabtcf.utils.BitReader;
import tech.bedrockstreaming.iabtcf.utils.BitSetIntIterable;
import tech.bedrockstreaming.iabtcf.utils.FieldDefs;
...
import tech.bedrockstreaming.iabtcf.v2.SegmentType;
...
class TCStringV2 implements TCString {
    ...
    private BitReader getSegment(SegmentType segmentType) {
        ...
        for (BitReader rbbv : remainingVectors) {
            int rSegmentType = rbbv.readBits3(FieldDefs.OOB_SEGMENT_TYPE);
    ...
    public IntIterable getPubPurposesConsent() {
        if (cache.add(FieldDefs.PPTC_PUB_PURPOSES_CONSENT)) {
            publisherPurposesConsent = BitSetIntIterable.EMPTY;

            BitReader dvBbv = getSegment(SegmentType.PUBLISHER_TC);
            if (dvBbv != null) {
                publisherPurposesConsent = fillBitSet(dvBbv, FieldDefs.PPTC_PUB_PURPOSES_CONSENT);
            }
        }
    ...
    public int getVersion() {
        if (cache.add(FieldDefs.CORE_VERSION)) {
            version = bbv.readBits6(FieldDefs.CORE_VERSION);
        }
        return version;
    }
    ...
    public Instant getCreated() {
        if (cache.add(FieldDefs.CORE_CREATED)) {
            consentRecordCreated = Instant.ofEpochMilli(bbv.readBits36(FieldDefs.CORE_CREATED) * 100);
        }
    ...
    public Instant getLastUpdated() {
        if (cache.add(FieldDefs.CORE_LAST_UPDATED)) {
            consentRecordLastUpdated = Instant.ofEpochMilli(bbv.readBits36(FieldDefs.CORE_LAST_UPDATED) * 100);
        }
    ...
    @Override
    public int getCmpId() {
        if (cache.add(FieldDefs.CORE_CMP_ID)) {
            consentManagerProviderId = (short) bbv.readBits12(FieldDefs.CORE_CMP_ID);
        }
    ...
    public int getCmpVersion() {
        if (cache.add(FieldDefs.CORE_CMP_VERSION)) {
            consentManagerProviderVersion = (short) bbv.readBits12(FieldDefs.CORE_CMP_VERSION);
        }
    ...
    public int getConsentScreen() {
        if (cache.add(FieldDefs.CORE_CONSENT_SCREEN)) {
            consentScreen = bbv.readBits6(FieldDefs.CORE_CONSENT_SCREEN);
        }
    ...
    public String getConsentLanguage() {
        if (cache.add(FieldDefs.CORE_CONSENT_LANGUAGE)) {
            consentLanguage = bbv.readStr2(FieldDefs.CORE_CONSENT_LANGUAGE);
        }
    ...
    public int getVendorListVersion() {
        if (cache.add(FieldDefs.CORE_VENDOR_LIST_VERSION)) {
            vendorListVersion = (short) bbv.readBits12(FieldDefs.CORE_VENDOR_LIST_VERSION);
        }
    ...
    public IntIterable getPurposesConsent() {
        if (cache.add(FieldDefs.CORE_PURPOSES_CONSENT)) {
            purposesConsent = fillBitSet(bbv, FieldDefs.CORE_PURPOSES_CONSENT);
        }
    ...
    public IntIterable getVendorConsent() {
        if (cache.add(FieldDefs.CORE_VENDOR_BITRANGE_FIELD)) {
            vendorConsents = fillVendors(bbv, FieldDefs.CORE_VENDOR_MAX_VENDOR_ID, FieldDefs.CORE_VENDOR_BITRANGE_FIELD);
        }
    ...
    public int getTcfPolicyVersion() {
        if (cache.add(FieldDefs.CORE_TCF_POLICY_VERSION)) {
            policyVersion = bbv.readBits6(FieldDefs.CORE_TCF_POLICY_VERSION);
        }
    ...
    public boolean isServiceSpecific() {
        if (cache.add(FieldDefs.CORE_IS_SERVICE_SPECIFIC)) {
            isServiceSpecific = bbv.readBits1(FieldDefs.CORE_IS_SERVICE_SPECIFIC);
        }
    ...
    public boolean getUseNonStandardStacks() {
        if (cache.add(FieldDefs.CORE_USE_NON_STANDARD_STOCKS)) {
            useNonStandardStacks = bbv.readBits1(FieldDefs.CORE_USE_NON_STANDARD_STOCKS);
        }
    ...
    public IntIterable getSpecialFeatureOptIns() {
        if (cache.add(FieldDefs.CORE_SPECIAL_FEATURE_OPT_INS)) {
            specialFeaturesOptInts = fillBitSet(bbv, FieldDefs.CORE_SPECIAL_FEATURE_OPT_INS);
        }
    ...
    public IntIterable getPurposesLITransparency() {
        if (cache.add(FieldDefs.CORE_PURPOSES_LI_TRANSPARENCY)) {
            purposesLITransparency = fillBitSet(bbv, FieldDefs.CORE_PURPOSES_LI_TRANSPARENCY);
        }
    ...
    public boolean getPurposeOneTreatment() {
        if (cache.add(FieldDefs.CORE_PURPOSE_ONE_TREATMENT)) {
            isPurposeOneTreatment = bbv.readBits1(FieldDefs.CORE_PURPOSE_ONE_TREATMENT);
        }
    ...
    public String getPublisherCC() {
        if (cache.add(FieldDefs.CORE_PUBLISHER_CC)) {
            publisherCountryCode = bbv.readStr2(FieldDefs.CORE_PUBLISHER_CC);
        }
    ...
    public IntIterable getVendorLegitimateInterest() {
        if (cache.add(FieldDefs.CORE_VENDOR_LI_BITRANGE_FIELD)) {
            vendorLegitimateInterests =
                    fillVendors(bbv, FieldDefs.CORE_VENDOR_LI_MAX_VENDOR_ID, FieldDefs.CORE_VENDOR_LI_BITRANGE_FIELD);
        }
    ...
    public List<PublisherRestriction> getPublisherRestrictions() {
        if (cache.add(FieldDefs.CORE_PUB_RESTRICTION_ENTRY)) {
    ...
    public IntIterable getAllowedVendors() {
        if (cache.add(FieldDefs.AV_VENDOR_BITRANGE_FIELD)) {
            allowedVendors = BitSetIntIterable.EMPTY;

            BitReader dvBbv = getSegment(SegmentType.ALLOWED_VENDOR);
            if (dvBbv != null) {
                allowedVendors = fillVendors(dvBbv, FieldDefs.AV_MAX_VENDOR_ID, FieldDefs.AV_VENDOR_BITRANGE_FIELD);
            }
        }
    ...
    public IntIterable getDisclosedVendors() {
        if (cache.add(FieldDefs.DV_VENDOR_BITRANGE_FIELD)) {
            disclosedVendors = BitSetIntIterable.EMPTY;

            BitReader dvBbv = getSegment(SegmentType.DISCLOSED_VENDOR);

            if (dvBbv != null) {
                disclosedVendors = fillVendors(dvBbv, FieldDefs.DV_MAX_VENDOR_ID, FieldDefs.DV_VENDOR_BITRANGE_FIELD);
            }
        }
    ...
    public IntIterable getPubPurposesLITransparency() {
        if (cache.add(FieldDefs.PPTC_PUB_PURPOSES_LI_TRANSPARENCY)) {
            publisherPurposesLITransparency = BitSetIntIterable.EMPTY;

            BitReader dvBbv = getSegment(SegmentType.PUBLISHER_TC);
            if (dvBbv != null) {
                publisherPurposesLITransparency = fillBitSet(dvBbv, FieldDefs.PPTC_PUB_PURPOSES_LI_TRANSPARENCY);
            }
        }
    ...
    public IntIterable getCustomPurposesConsent() {
        if (cache.add(FieldDefs.PPTC_CUSTOM_PURPOSES_CONSENT)) {
            customPurposesConsent = BitSetIntIterable.EMPTY;

            BitReader dvBbv = getSegment(SegmentType.PUBLISHER_TC);
            if (dvBbv != null) {
                customPurposesConsent = fillBitSet(dvBbv, FieldDefs.PPTC_CUSTOM_PURPOSES_CONSENT);
            }
        }
    ...
    public IntIterable getCustomPurposesLITransparency() {
        if (cache.add(FieldDefs.PPTC_CUSTOM_PURPOSES_LI_TRANSPARENCY)) {
            customPurposesLITransparency = BitSetIntIterable.EMPTY;

            BitReader dvBbv = getSegment(SegmentType.PUBLISHER_TC);
            if (dvBbv != null) {
                customPurposesLITransparency = fillBitSet(dvBbv, FieldDefs.PPTC_CUSTOM_PURPOSES_LI_TRANSPARENCY);
            }
        }
